//////////////////////////////////////////////////////////////////////
// AuthAccount - Mis à jour pour Google Identity Services (GIS) & GAPI
//////////////////////////////////////////////////////////////////////
...
AuthAccount.prototype.Connect = function() {
    ...
    // 1. Initialisation de gapi.client pour les requêtes Drive / Calendar
    gapi.load('client', function() {
        gapi.client.init({
            // Pas besoin de clientId ici avec GIS, gapi.client gérera le token via setToken
        }).then(function() {
            // 2. Initialisation du Token Client de Google Identity Services
            if (typeof google !== 'undefined' && google.accounts) {
                self.tokenClient = google.accounts.oauth2.initTokenClient({
                    ...
                    callback: function(tokenResponse) {
                        ...
                        // Récupération optionnelle de l'email via l'API Userinfo ou stockage d'un état générique
                        self.fetchUserEmail();
                    }
                });
                
                // Vérifier si un bouton de connexion UI doit être affiché
                self.renderGISButton();
            } else {
...
AuthAccount.prototype.renderGISButton = function() {
    var btnContainer = document.getElementById("g_signbtn");
    if (btnContainer && typeof google !== 'undefined') {
        try {
            ...
            console.warn("GIS renderButton skipped or failed: ", e);
        }
    }
}

AuthAccount.prototype.handleCredentialResponse = function(response) {
    // Si l'authentification One Tap / Bouton GSI standard est utilisée, 
    // on déclenche ensuite le flux OAuth scopes pour Calendar/Drive si nécessaire.
    if (this.tokenClient) {
...
AuthAccount.prototype.fetchUserEmail = function() {
    var self = this;
    // Récupération de l'email de l'utilisateur connecté via l'API OIDC/Userinfo de Google
    ...
    xhr.onerror = function() {
        // Fallback si l'appel userinfo échoue mais que le token est valide
        self.onSignIn();
        self.postSignInWatch();
    };
    xhr.send();
...
AuthAccount.prototype.SignIn = function() {
    if (this.tokenClient) {
        // Ouvre la popup de consentement Google OAuth 2.0
...
AuthAccount.prototype.SignOut = function() {
    if (this.access_token && typeof google !== 'undefined') {
        google.accounts.oauth2.revoke(this.access_token, function() {
            console.log('Token révélé / Déconnecté');
        });
    }
    this.access_token = null;
    this.userEmail = null;
...
AuthAccount.prototype.Fail = function(reason) {
    ...
    if (reason && reason.error) {
        ...
        msg = "Erreur d'authentification inconnue";
    }
...
//////////////////////////////////////////////////////////////////////
// AuthAppData (Inchangé dans sa structure, s'appuie sur gapi.client)
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
// AuthCal (Inchangé, utilise les requêtes standard de gapi.client)
//////////////////////////////////////////////////////////////////////
